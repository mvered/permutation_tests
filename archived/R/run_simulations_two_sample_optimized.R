...
run_two_sample_suite <- function(x_pool, y_pool, label) {
  ...
  results_list <- lapply(n_obs, function(n_val) {
    ...
    sim_data <- future_lapply(1:n_sims, function(i) {
      ...
      return(c(
        std_05  = as.numeric(p_vals["standard"] < 0.05),
        std_10  = as.numeric(p_vals["standard"] < 0.10),
        perm_05 = as.numeric(p_vals["permutation"] < 0.05),
        perm_10 = as.numeric(p_vals["permutation"] < 0.10)
      ))
    ...
    return(data.frame(
      Observations = n_val,
      standard_05    = rates["std_05"],
      standard_10    = rates["std_10"],
      permutation_05 = rates["perm_05"],
      permutation_10 = rates["perm_10"]
    ))
  })
